...
plot_2Dspecta = function(spec,time=1L,normalize=TRUE,trim=0.01,cut_off=.4,...){
  ...
  ggplot(df,aes(xmin=frequency1,xmax=frequency2,ymin=dir-5,ymax=dir+5,fill=ef,col=ef))+
    geom_rect()+
    ...
    labs(title=paste('Directionnal Wave energy spectrum at location',spec$station),subtitle=format(spec$forcings$time[time],format="%Y-%m-%d %H:%M"),caption="Source: Resourcecode hindcast database\nresourcecode.ifremer.fr")
}
