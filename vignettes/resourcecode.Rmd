...
For example, if one is interested in the time series of sea-state parameters some location, the following code can be adapted:
...
- 1D and 2D spectra of the SPEC grid can be download directly from IFREMER FTP using functions `get_1Dspectrum()` and `get_2Dspectrum()`. We also provide a plotting function for the 2D spectrum.

```{r}
node_SPEC = closest_point_SPEC(point_of_interest)
```

Then one can extract the 1D spectrum time-series and see the contents of this downloaded data.
...
in addition, it is possible to plot the wave elevation directional spectra for any given time, which can be specified by the time index of directly the date:
